...
- [x] **Rule-based smart playlists.** Field + operator + value (artist is X, genre contains Y, play count > Z) instead of only hand-built M3U lists. Built 2026-09-14, merged to `master` 2026-09-15 after a full codebase review/cleanup pass (bug fixes, per-field-type rule value widgets, 0-match guard, save-as-static now converts instead of duplicating) — see `docs/superpowers/specs/2026-09-14-smart-playlists-design.md`, `docs/superpowers/plans/2026-09-14-smart-playlists.md`, and `docs/issues/issues_track.md`. **Decided 2026-09-15**: leaving the `year is X` mixed-genre behavior as-is, no companion filter or extra guidance. It's correct per spec — the user still has the choice to stack a `genre` rule alongside `year` if they want it narrower; no change needed.
- [x] **A real equalizer + loudness normalization.** Built 2026-09-15 — 5-band EQ (60/250/1k/4k/12k Hz, ±12dB) + presets (Flat/Rock/Pop/Bass Boost/Vocal) + ReplayGain-based loudness normalization (embedded tags + lazy-compute fallback), modal UI off a new player-bar button. Made the audio graph permanent in the process (previously only existed while the visualizer had been toggled on). Also fixed a real pre-existing bug hit along the way: an upsert-triggered `UNIQUE constraint failed: artists.name` crash on rescans. See `docs/superpowers/specs/2026-09-15-equalizer-design.md` and `docs/issues/issues_track.md`. **Open:** live Electron smoke test not yet done; not yet committed.
