...
- [x] **Rule-based smart playlists.** Field + operator + value (artist is X, genre contains Y, play count > Z) instead of only hand-built M3U lists. Built and live-verified 2026-09-14 on branch `feature/smart-playlists` (not yet merged to master) — see `docs/superpowers/specs/2026-09-14-smart-playlists-design.md`, `docs/superpowers/plans/2026-09-14-smart-playlists.md`, and `docs/issues/issues_track.md`. Open UX question carried to next session: a lone `year is X` rule pulls in every genre tagged that year (correct per spec, but Erich wasn't happy with the mixed-genre result on a real test) — decide whether that needs a companion filter or better guidance in the rule builder.
