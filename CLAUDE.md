...
#### Up Next Pane Falls Back to the Loaded Playlist
`UIController.renderQueuePane()` no longer shows "Nothing queued" just because nothing was manually dragged into the queue. It renders `CoreAudio.queue` (manual, plays first) followed by `CoreAudio.playlist.slice(currentTrackIndex + 1)` (the rest of whatever's actually loaded — dimmed, not draggable/removable, just a preview). `CoreAudio.playSong()` calls `notifyQueueChanged()` after every track load so this stays live as playback advances, not just on explicit queue edits. This means playing a folder/playlist populates "Up Next" with no dedicated wiring required — don't reintroduce a separate "feed the queue" mechanism for Play All-style actions, this fallback already covers it.
